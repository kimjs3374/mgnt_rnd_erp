 ...
 *
 * **사용자가 준 실제 양식 그대로 만든다.** 한 사람이 두 줄이고, 자격·구분·지급구분·총액·급여총액은
 * 그 두 줄에 걸쳐 세로로 병합된다. 위 두 줄은 머리글이고 역시 병합된다.
 *
 *   자격 | 구분(내외부) | 성명        | 소속기관명 | 직급(직위) | 신규채용여부 | 참여시작일  | 참여종료일   | 지급구분 | 총액 | 급여 총액
  ...
  sheet.push([
    {
      v:
        ...
        ` · 지급=현금 · 미지급=현물 · 내려받은 시각 ${new Date().toISOString().slice(0, 16).replace("T", " ")} UTC`,
    },
  ])
