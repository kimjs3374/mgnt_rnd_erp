...
# 「요건미확인」도 넣는다(2026-09-04). 원래 뺐던 이유는 판정계산()이 가능/불가가 아닌 값을
# 전부 「확인필요」로 묶어 등급 구분이 사라진다는 것이었는데, 그 로직을 고쳐 이제 그대로
# 통과시킨다(lib/queries.ts·lib/queries-programs.ts).
#
# ⚠ 안 넣으면 더 나쁜 일이 생긴다 — 결정 행이 없는 공고는 화면 계산이 마지막 단계(지역·
#   지원대상만 대조)로 떨어져 **「가능」으로 올라간다.** 실측(사용자 신고 2026-09-04):
#   엔진은 가능 10건이라 했는데 화면엔 30건이 떴다. 차이 20건이 전부 「요건미확인」
#   (본문을 못 읽어 판정을 못 한 건)이었다. 못 읽었다는 사실을 화면이 「가능」으로
#   바꿔 말하면 안 된다.
INCLUDE = ("가능", "불가", "확인필요", "해당없음", "요건미확인")
