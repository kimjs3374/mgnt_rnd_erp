...
import { ProgramChecksPanel } from "@/components/program-checks-panel"
...
/**
 * 지원사업의 **단계별 화면 한 벌** — 신청중 · 신청완료 · 수행중 · 사업종료 (+ 전체).
 *
...
export async function ProgramsStageView({ 단계 }: { 단계: 사업단계 | "전체" }) {
  const [{ rows: 전체행, error }, 종료일행, 점검목록] = await Promise.all([
    getLedger(),
  ...
  return (
    ...
    >
      ...
      {점검목록.error && <DbError what="제출 전 점검" error={점검목록.error} />}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat
          ...
          label="사업 수"
          value={rows.length}
          // 숫자 아래 글은 짧게(2026-09-04 사용자 지시) — 정의.설명(페이지 설명)을
          // 카드에 다시 길게 반복하지 않는다.
          sub={
            전체보기중
              ? "검토·심사·수행·종료"
