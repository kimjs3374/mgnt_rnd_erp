...
export function SettlementDeadlineCard({ 정산 }: { 정산: 정산표시 }) {
  ...
  // 숫자 아래 글은 짧게(2026-09-04 사용자 지시) — 왜 당겨졌는지 같은 자세한 사정은
  // 여기서 다 안 적는다. 확인이 꼭 필요한 것만 짧은 표로 남기고, 나머지는 「편집」을
  // 펼치면 그대로 있다(정산.이유·정산.규칙 등 원본값은 그대로 넘어온다).
  const 설명 =
    (정산.달지정
      ? `${정산.날}(${정산.요일}) · 이번 달만`
      : `${정산.날}(${정산.요일}) · 매월 ${정산.규칙.기준일}일`) +
    (정산.옮겨짐 ? ` · ${이동설명[정산.규칙.이동]}` : "") +
    (정산.확인필요 ? " · 확인 필요" : "") +
    (정산.기본값사용 ? " · 기본값" : "")
