...
function DDay({ 남은일 }: { 남은일: number | null }) {
  if (남은일 == null) return <span className="text-[11.5px] text-muted-foreground">상시·미정</span>
  if (남은일 < 0)
    return <span className="text-[11.5px] text-muted-foreground">마감 {-남은일}일 지남</span>
  const 급함 = 남은일 <= 7
  return (
    <span
      className={
        "inline-flex h-5 items-center rounded-4xl px-2 text-[11.5px] font-medium tabular-nums " +
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border bg-card p-4">
        <div className="mb-1 text-[13px] font-medium">관심 공고</div>
  ...
  return (
    <div className="rounded-lg border bg-card p-4">
      <div className="mb-2 flex flex-wrap items-baseline gap-2">
        <span className="text-[13px] font-medium">관심 공고</span>
      ...
      <ul className="flex flex-col gap-1.5">
        {rows.map((r) => (
          <li
            key={r.공고_id}
            className="flex flex-wrap items-center gap-x-2 gap-y-1 rounded-md border px-3 py-2 text-[12.5px]"
          >
            <DDay 남은일={r.남은일} />
            ...
            {r.지원과제 == null ? (
              <span className="ml-auto text-[11.5px] text-[var(--warning-fg)]">
                아직 지원 등록 안 함 —{" "}
            ...
            {r.메모 && (
              <span className="basis-full text-[11px] text-muted-foreground">메모: {r.메모}</span>
            )}
          </li>
        ))}
      </ul>
