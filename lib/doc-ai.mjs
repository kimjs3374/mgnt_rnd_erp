...
/**
 * 헤드리스 모델. 기본은 **haiku** 다 — 2026-09-03 실측으로 낮췄다.
 *
 * 왜 haiku 로 충분한가: 이 파일이 하는 일은 서류에서 적힌 값을 그대로 옮기는 것이다.
 * 추론이 아니라 전사(轉寫)라서 큰 모델이 이길 자리가 아니다. 실측(발급일·발급기관·
 * 서류종류 판독)에서 sonnet 과 같은 답에 확신도 0.95 를 냈다.
 *
 * 되돌리려면 코드를 고치지 말고 `/web/rnd/.env.local` 에 한 줄을 둔다:
 *   RND_DOC_MODEL=claude-sonnet-5
 * 판독이 이상해지면 여기부터 되돌린다.
 */
const MODEL = process.env.RND_DOC_MODEL || "claude-haiku-4-5-20251001"
