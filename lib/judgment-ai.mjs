...
//
// 라우트: POST /judgment/record · POST /judgment/similar · POST /judgment/history (bot/gateway.py)
...
/**
 * 이 공고에 실제로 남긴 판정+코멘트 이력. 비슷한사례()와 달리 임베딩 유사도가
 * 아니라 announcement_id 로 정확히 필터한다 — 임베딩을 계산하지 않으니 빠르고,
 * 백그라운드 fill_embedding() 이 아직 안 끝났어도 바로 보인다.
 */
export async function 판정이력(announcementId) {
  const r = await call("/judgment/history", { announcement_id: announcementId })
  if (!r.ok) return { ok: false, error: r.error ?? "조회 실패", rows: [] }
