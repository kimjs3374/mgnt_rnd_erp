...
export const NAV: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // ⚠ 「사업 대장」 하나였던 것을 **단계 셋**으로 나눴다(2026-09-03 사용자 지시).
      //    한 표에 신청·수행·종료가 섞여 있으면 「지금 뭘 해야 하나」가 안 보인다 —
      //    신청중은 결과를 기다리는 일이고, 수행중은 돈을 쓰는 일이고, 종료는 정산·보고가 남은 일이라
      //    할 일의 성격이 아예 다르다.
      //    단계는 **저장하지 않고 계산한다**(`lib/project-stage.ts`) — 그래야 선정을 기록하는 순간
      //    수행중으로, 수행기간이 지나면 사업종료로 저절로 넘어간다.
      { title: "신청중", url: "/projects/applying" },
      // 공고 탐색과 수행중 **사이**에 둔다. 일의 순서가 그렇다 —
      // 공고를 보고 → 지원해서 선정되면 → 여기서 사업비를 잡고 → 그 결과가 대장에 쌓인다.
      // 아래에 두면 「이미 끝난 것을 들여다보는 화면」으로 읽힌다.
