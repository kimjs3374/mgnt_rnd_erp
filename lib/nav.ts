...
export const NAV: NavGroup[] = [
  ...
  // ⚠ 「판정 리포트」는 사이드바에서 뺐다(2026-09-04 사용자 지시). 화면·주소는 그대로
  //   살아 있다(`/engine`) — 규칙 엔진이 무엇을 어떻게 걸렀는지 보고 접힌 것을 되돌리는
  //   화면이라 지우지 않았다. 되돌리려면 이 자리에 아래를 넣는다.
  //     { title: "판정 리포트", url: "/engine", icon: "PieChart" },
  //   (아이콘은 components/app-sidebar.tsx 의 ICONS 에 이미 있는 것만 쓴다.
  //    lib/access.ts 에는 접두사가 없다 — 부서 무관 공용이라 기본이 허용이다.)
  {
    title: "회사",
    url: "/company",
    icon: "Building2",
