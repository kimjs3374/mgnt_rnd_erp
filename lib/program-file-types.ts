...
export type 사업파일 = {
  ...
  지출?: {
    ...
    합계: number | null
    /**
     * 어느 비목으로 쓴 돈인가 — 「연구시설·장비 및 재료비 › 연구재료 구입비」 꼴.
     * 접힌 줄에서 바로 보이게 한다(2026-09-04 사용자 지시: "어떤 비목으로 썼는지도
     * 간략하게 알려줘"). 세부항목이 비어 있으면 대분류만 담는다 — 실측상 미지정이
     * 흔하고, 줄마다 「(미지정)」을 찍으면 그게 더 시끄럽다.
     */
    비목: string | null
  }
}
...
export type 지출묶음 = {
  ...
  일자: string | null
  합계: number | null
  /** 「연구시설·장비 및 재료비 › 연구재료 구입비」. 세부항목이 없으면 대분류만. */
  비목: string | null
  파일: 사업파일[]
}
