...
export type 사업파일 = {
  ...
  업로더: string | null
  /**
   * 「집행 증빙」일 때만 있다 — 그 파일이 붙은 집행(지출) 건. 화면이 이 값으로 파일을
   * 지출 하나로 묶어 접는다(2026-09-04 사용자 지시: "파일이 개별로 보이니 너무 복잡하니까
   * 지출 하나만 보이고 마우스로 누르면 펼쳐서 볼 수 있게"). 계상 증빙·정산 서류는 지출이라는
   * 개념이 없어(비목·서류종류에 붙지, 거래 건에 붙지 않는다) 그대로 낱개로 보여준다.
   */
 ...
 *
 * 「집행 증빙」만 지출이 있다 — 그것들만 지출_id 로 묶고, 계상 증빙·정산 서류는
 * 지출 개념이 없어(비목·서류종류에 붙지 거래 건에 붙지 않는다) 그대로 낱개로 남긴다.
