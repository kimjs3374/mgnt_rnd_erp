...
try {
  ...
  // 12차 개편: 제출 전 점검 상세에 「외 N건」이 다시 등장한다 — 이번엔 줄 전체가
  // 링크라 눌린다. 그래서 텍스트 존재 자체가 아니라 "있으면 링크 안에 있는가"를 본다.
  const 외건검사 = await p.evaluate(() => {
    const h2 = [...document.querySelectorAll("h2")].find((h) => h.textContent.trim() === "확인 및 조치")
    const card = h2?.closest("div.rounded-lg")
    const 후보 = [...(card?.querySelectorAll("*") ?? [])].filter(
      (el) => el.children.length === 0 && /외\s*\d+\s*건/.test(el.textContent ?? ""),
    )
    return { 개수: 후보.length, 전부링크안: 후보.every((el) => el.closest("a") !== null) }
  })
  확인(
    "확인 및 조치의 「외 N건」은 없거나, 있으면 링크 안에 있음(안 눌리는 텍스트 아님)",
    외건검사.개수 === 0 || 외건검사.전부링크안,
    `${외건검사.개수}개`,
  )
  ...
  if (오늘카드.includes("서류")) {
    확인(
      ...
      오늘카드.includes("발급 필요") || 오늘카드.includes("갱신 필요"),
  ...
  if (오늘카드.includes("점검") && !오늘카드.includes("지금 손댈 것이 없습니다")) {
    // 12차 개편: 건수 뭉치("N건") 대신 무엇이 문제인지(종류·대상)를 보여준다.
    // `scripts/check-programs.mjs` 가 매기는 종류는 "기한임박"·"금액불일치" 둘뿐이다.
    확인(
      "제출 전 점검 - 「미해결」과 무엇이 문제인지(종류)가 같이 뜸",
      오늘카드.includes("미해결") &&
        ["기한임박", "금액불일치"].some((종류) => 오늘카드.includes(종류)),
      오늘카드.replace(/\s+/g, " ").trim().slice(0, 100),
    )
  }
