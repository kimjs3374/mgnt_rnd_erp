...
try {
  ...
  // 과제 13 은 2022-06-01~2024-05-31 — **기간은 2년이지만 3개 연차**다(회계연도로 센다).
  // 「협약 2년」이라고 쓰던 것을 고친 자리다. 연도까지 같이 보여야 오해가 안 생긴다.
  const 협약안내 = await page.evaluate(
    () => /협약 \d+개 연차[^\n]*/.exec(document.body.innerText)?.[0] ?? null,
  )
  log(`${협약안내 === "협약 3개 연차 (2022 · 2023 · 2024)" ? "✓" : "✗"} 협약 안내: ${협약안내 ?? "없음"}`)
  ...
  // 탭에 회계연도가 붙어야 「2차년도가 2023년」임이 보인다. 숫자만 있으면 또 기간으로 오해한다.
  log(
    `${탭[0]?.startsWith("1차년도2022") && 탭[1]?.startsWith("2차년도2023") ? "✓" : "✗"} 탭에 회계연도가 붙는다`,
  )
