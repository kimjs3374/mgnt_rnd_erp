...
try {
  ...
  if (사이드바) {
    // ⚠ 예전엔 사이드바에 단계 셋이 다 있었는데 **간소화되면서 「전체」만 남았다**(커밋 31baf8e).
    //   단계 사이 이동은 이제 화면 위쪽 **칩**이 한다 — 아래 ①에서 그걸 검사한다.
    //   그래서 여기서는 「그룹이 있고 들어갈 입구가 있다」까지만 본다.
    확인(
      사이드바.항목.length > 0,
      `그 그룹에 들어갈 항목이 있다 (${사이드바.항목.join(" · ")})`,
    )
    const 셋 = ["신청중", "수행중", "사업종료"]
    확인(
      셋.every((t) => !사이드바.과제사업.includes(t)),
      `과제사업 그룹에는 단계가 안 남아 있다 (${사이드바.과제사업.join(" · ") || "비어 있음"})`,
    )
    // 없앤 화면이 메뉴에 남아 있으면 눌렀을 때 아무 데도 못 간다 — 실제로 한 번 그랬다.
    확인(
      !사이드바.과제사업.includes("과제 계상"),
      "없앤 「과제 계상」이 메뉴에 안 남아 있다",
    )
  }
